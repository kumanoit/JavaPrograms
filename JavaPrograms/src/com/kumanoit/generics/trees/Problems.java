...
public class Problems {
	...
	private static <T> void getTreeWidth(TreeNode<T> root, Map<Integer, Integer> nodeCountMap, int level) {
		if (root == null) {
			return;
		}
	...
	private static <T> TreeNode<T> constructTreePreOrderInOrder(T[] pre, T[] in, int preIndex, int inIndex, int size) {
		if (size == 0) {
			return null;
		}
		int i = inIndex;
		...
		root.setLeftChild(constructTreePreOrderInOrder(pre, in, preIndex + 1, inIndex, i));
		root.setRightChild(constructTreePreOrderInOrder(pre, in, preIndex + i + 1, inIndex + i + 1, size - i - 1));
		return root;
	}

	public static <T> int evaluateExpression(TreeNode<T> root) {
		if (root == null) {
			return -1;
		}
		String value = root.getValue().toString();
		if (value.compareTo("0") >= 0 && value.compareTo("9") <= 9) {
			return Integer.parseInt(value);
		}
		int leftValue = evaluateExpression(root.getLeftChild());
		int rightValue = evaluateExpression(root.getRightChild());
